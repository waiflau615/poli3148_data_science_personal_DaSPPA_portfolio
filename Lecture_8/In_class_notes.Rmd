---
title: "Lecture 8 in class notes"
...
`{summary(d$region) # Quick summary # not informative for categorical variable} length(d$region) # Number of observations is.na(d$region) |> sum() # Number of missing values`

## Summary Statistics (con'd)

```{r}
table(d$region, useNA = "always")
table(d$region, useNA = "always") |> as.data.frame() |> as.tibble()
...
d |> group_by(region) |> count() |> ungroup() |>
  mutate(prop = n / sum(n))
```
...
When encountering long text from many category --\> flip them using either i or ii method
...
```{r, fig.height=4}
# re-order the frequency (based on freq)
...
```{r, fig.height=4}
# fct_rev = reorder from lower to large
...
know the proportion of democracy and non-democracy in each region
...
```{r}
# position_stack tell r to put it in middle of the bar
d |> group_by(region_higher, democracy_binary) |> summarise(n_obs = n()) |>
  mutate(democracy_binary = case_match(democracy_binary, 1 ~ "Yes", 0 ~ "No", NA ~ NA)) |>
  ggplot(aes(x = n_obs, y = region_higher, fill = democracy_binary)) +
  geom_bar(stat = "identity", position = "stack") +
...
d |> ggplot(aes(x = gdppc, y = life_expectancy)) +
  ...
  geom_smooth() +
  labs(x = "GDP per capita", y = "Life Expectancy", color = "Region", shape = "Region",
       title = "Wealth and Health in the World (1800-2019)") # + scale_x_sqrt()
```
...
# scale = how to measure the scale (based on actual data of column)
```
