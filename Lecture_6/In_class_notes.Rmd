...
#### only look for columns with the same names --merge with top to bottom (either vertical;y or horizonally)

Include the columns that appears in either dataset, if didn't appear in one of them, all values in that dataset will NA.
...
# require all dataset should have same number of rows
...
-   If two dataset using two different sorting by arrange function, not work even if they are same number of rows and location

-   If you have any slightest doubt, don't use them.

-   `bind_rows` are more safety option

# Join Tables

## Tasks

more frequently used than the `bind_col` functions.
...
```{r}
# first rows is left side
# merge with the dataset that appears in left hand side
# throw away the years from 1984 to 1999, everything data that appears in left side will merge
...
```{r}
# assign NA when there is no data appears in right side
...
```{r}
# merage data that appears in both dataset
...
```{r}
# merge data that appear in either dataset
...
```{r}
# check whether data appears in right hand side -- if does, keep the record ; if not throw that record
...
```{r}
# check if the identifers appears in right side, if does remove them
# remove the data(year) that right side cover
...
Add **binary indicators** about data availability in each sources. --\> **help with trouble shooting for following two reason**

**A column can Missing when merging** can be due to **(i.)** it doesn't exist in either dataset (whole record did not appear, i.e. that country and year) **(ii.).** that record exist but value in that record is missing

IF first issue --\> question the dataset integrity

if second, record as NA is fine
